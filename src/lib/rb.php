...
namespace RedBeanPHP\Logger {
    ...
    class RDefault implements Logger
    {
        ...
        {
            if (
                $mode !== self::C_LOGGER_ARRAY &&
                $mode !== self::C_LOGGER_ECHO
            ) {
    ...
    class Debug extends RDefault implements Logger
    {
        ...
        protected function fillInValue($value)
        {
            ...
            if (
                ...
                if (
                    \RedBeanPHP\QueryWriter\AQueryWriter::canBeTreatedAsInt(
                        $value,
                    ) ||
                    $value === "NULL"
                ) {
    ...
    class RPDO implements Driver
    {
        ...
        protected function setEncoding()
        {
            ...
            if ($driver === "mysql") {
                ...
                $collate = $this->hasCap("utf8mb4_520")
                    ? "_unicode_520_ci"
                    : "_unicode_ci";
        ...
        public function __construct(
            ...
            if (
                (strpos($this->dsn, "sqlite") === 0 &&
                    PHP_MAJOR_VERSION === 5 &&
                    PHP_MINOR_VERSION === 3) ||
                defined("HHVM_VERSION") ||
                $this->dsn === "test-sqlite-53"
            ) {
        ...
        public function setUseStringOnlyBinding($yesNo)
        {
            ...
            if (
                $this->loggingEnabled &&
                $this->logger &&
                method_exists($this->logger, "setUseStringOnlyBinding")
            ) {
        ...
        public function connect()
        {
            ...
            try {
                ...
                $options = [];
                if (
                    isset($this->connectInfo["options"]) &&
                    is_array($this->connectInfo["options"])
                ) {
         ...
         * @return void
         */
        public function setPDO(\PDO $pdo, $options = [])
        {
            $this->pdo = $pdo;
        ...
        {
            return trim(
                strval($this->pdo->getAttribute(\PDO::ATTR_SERVER_VERSION)),
            );
        }
    }
}
        ...
        Jsonable
    {
        ...
        public static function setMetaAll($beans, $property, $value)
        {
            foreach ($beans as $bean) {
                ...
                if ($property == "type" && !empty($bean->beanHelper)) {
                    $bean->__info["model"] = $bean->beanHelper->getModelForBean(
                        $bean,
                    );
                }
            }
        ...
        private function getOwnList($type, $redbean)
        {
            ...
            if ($this->getID()) {
                ...
                $firstKey =
                    count($this->withParams) > 0 ? key($this->withParams) : 0;
                if (is_int($firstKey)) {
                ...
                $beans = $redbean->find($type, [], $sql, $bindings);
            }
        ...
        public function initializeForDispense($type, $beanhelper = null)
        {
            ...
            if ($beanhelper) {
                $this->__info["model"] = $this->beanHelper->getModelForBean(
                    $this,
                );
            }
        ...
        public function export(
            ...
            foreach ($this as $key => $value) {
                if (!$onlyMe && is_array($value)) {
                    ...
                    if ($hasFilters) {
                        //has to be on one line, otherwise code coverage miscounts as miss
                        if (
                            !in_array(
        ...
        public function __set($property, $value)
        {
            ...
            $fieldLink = $property . "_id";
            $isFieldLink =
                ($pos = strrpos($property, "_id")) !== false &&
                array_key_exists(
                    $fieldName = substr($property, 0, $pos),
                    $this->properties,
                );
        ...
        {
            return isset($this->__info["data.bundle"][$key])
        ...
        public function __call($method, $args)
        {
            ...
            if (!is_callable([$this->__info["model"], $method])) {
                ...
                foreach ($this->properties as $property => $value) {
                    if (is_scalar($value)) {
                        if (self::$enforceUTF8encoding) {
                            $list[$property] = mb_convert_encoding(
                                $value,
                                "UTF-8",
                                "UTF-8",
                            );
            ...
            if ($this->getID()) {
                ...
                if (!self::$useFluidCount) {
                    $count = $this->beanHelper
                        ->getToolbox()
                        ->getWriter()
                        ->queryRecordCount($type, [], $sql, $bindings);
                } else {
                    $count = $this->beanHelper
                        ->getToolbox()
                        ->getRedBean()
        ...
        {
            $json = $this->__call("@__jsonSerialize", []);
    ...
    class DBAdapter extends Observable implements Adapter
    {
        ...
        public function setOption($optionKey, $optionValue)
        {
            if (method_exists($this->db, $optionKey)) {
                call_user_func([$this->db, $optionKey], $optionValue);
                return true;
            }
            return false;
        }
...
namespace RedBeanPHP {
    ...
    class BeanCollection
    {
        ...
        public function next()
        {
            ...
            if ($row) {
                $beans = $this->repository->convertToBeans(
                    $this->type,
                    [$row],
                    $this->mask,
                );
...
namespace RedBeanPHP {
    ...
    interface QueryWriter
    {
         ...
         * @return Cursor
         */
        public function queryRecordWithCursor(
            $type,
            $addSql = null,
            $bindings = []
        );
         ...
         * @return integer
         */
        public function queryCountTagged(
            $type,
            $tagList,
            $all = false,
            $addSql = "",
         ...
         * @return array
         */
        public function queryRecursiveCommonTableExpression(
            $type,
            $id,
            $up = true,
            $addSql = null,
            $bindings = [],
            $select = QueryWriter::C_CTE_SELECT_NORMAL
        );
         ...
         * @return boolean
         */
        public function sqlStateIn($state, $list, $extraDriverDetails = []);
    ...
    abstract class AQueryWriter
    {
        ...
        private function makeSQLFromConditions(
            ...
            foreach ($conditions as $column => $values) {
                ...
                if (
                    $checkOODB instanceof OODBBean &&
                    $checkOODB->getMeta("type") === $column &&
                    substr($column, -3) != "_id"
                ) {
                    $column = $column . "_id";
                }
            ...
            $addSql = $this->glueSQLCondition(
                $addSql,
                !empty($sqlConditions) ? QueryWriter::C_GLUE_AND : null,
            );
            if ($addSql) {
                $sql .= $addSql;
            }
        ...
        protected function getSQLFilterSnippet($type)
        {
            ...
            $sqlFilters = [];
            if (
                isset(self::$sqlFilters[QueryWriter::C_SQLFILTER_READ][$type])
            ) {
                foreach (
                    self::$sqlFilters[QueryWriter::C_SQLFILTER_READ][$type]
                    as $property => $sqlFilter
                ) {
                    if (
                        !self::$flagSQLFilterSafeMode ||
                        isset($existingCols[$property])
                    ) {
                        $sqlFilters[] = $sqlFilter . " AS " . $property . " ";
                    }
                }
            }
            $sqlFilterStr = count($sqlFilters)
                ? "," . implode(",", $sqlFilters)
                : "";
            return $sqlFilterStr;
        }
        ...
        public function glueLimitOne($sql = "")
        {
            return strpos(strtoupper(" " . $sql), " LIMIT ") === false
                ? $sql . " LIMIT 1 "
                : $sql;
        }
        ...
        public function updateRecord($type, $updatevalues, $id = null)
        {
            ...
            foreach ($updatevalues as $uv) {
                if (isset(self::$sqlFilters["w"][$type][$uv["property"]])) {
                    $p[] =
                        " {$this->esc($uv["property"])} = " .
                        self::$sqlFilters["w"][$type][$uv["property"]];
                } else {
        ...
        public function parseJoin($type, $sql, $cteType = null)
        {
            ...
            foreach ($expressions as $exp) {
                ...
                $sql = str_replace(
                    "@{$exp}",
                    "{$lastJoin}__rb{$nsuffix}.{$lastPart}",
                    $sql,
                );
            ...
            if (strpos($sql, " WHERE ") === false) {
                if (
                    preg_match(
                        "/^(ORDER|GROUP|HAVING|LIMIT|OFFSET)\s+/i",
                        trim($sql),
                    )
                ) {
                    $sql = "{$joinSql} {$sql}";
        ...
        public function queryRecord(
            ...
            if (
                $this->flagUseCache &&
                $this->sqlSelectSnippet != self::C_SELECT_SNIPPET_FOR_UPDATE
            ) {
        ...
        public function queryRecordLink(
            ...
            $sqlFilterStr = "";
            if (count(self::$sqlFilters)) {
                $linkType = $this->getAssocTable([$sourceType, $destType]);
         ...
         * @return array
         */
        private function queryTaggedGeneric(
            $type,
            $tagList,
            $all = false,
            $addSql = "",
        ...
        public function queryRecursiveCommonTableExpression(
            ...
            $direction = $up
                ? " {$alias}.{$type}_id = {$type}.id "
                : " {$alias}.id = {$type}.{$type}_id ";
        ...
        public function flushCache(
            $newMaxCacheSizePerType = null,
            $countCache = true
        ) {
            if (
                !is_null($newMaxCacheSizePerType) &&
                $newMaxCacheSizePerType > 0
            ) {
...
namespace RedBeanPHP\QueryWriter {
    ...
    class MySQL extends AQueryWriter implements QueryWriter
    {
         ...
         */
        public function __construct(Adapter $adapter, $options = [])
        {
            $this->typeno_sqltype = [
        ...
        public function useFeature($name)
        {
            if ($name == "ignoreDisplayWidth") {
                $this->typeno_sqltype[MySQL::C_DATATYPE_BOOL] =
                    " TINYINT UNSIGNED ";
                $this->typeno_sqltype[MySQL::C_DATATYPE_UINT32] =
                    " INT UNSIGNED ";
                foreach ($this->typeno_sqltype as $k => $v) {
                    $this->sqltype_typeno[trim(strtolower($v))] = $k;
                }
        ...
        public function addFK(
            ...
            if (
                !is_null(
                    $this->getForeignKeyForTypeProperty($tableNoQ, $fieldNoQ),
                )
            ) {
                return false;
            }
        ...
        public function sqlStateIn($state, $list, $extraDriverDetails = [])
        {
            ...
            if ($state == "HY000" && !empty($extraDriverDetails[1])) {
                ...
                if (
                    $driverCode == "1205" &&
                    in_array(QueryWriter::C_SQLSTATE_LOCK_TIMEOUT, $list)
                ) {
                    return true;
                }
            }
...
namespace RedBeanPHP\QueryWriter {
    ...
    class SQLiteT extends AQueryWriter implements QueryWriter
    {
        ...
        protected function buildFK(
            ...
            if (
                !is_null($this->getForeignKeyForTypeProperty($table, $column))
            ) {
                return false;
            }
        ...
        public function addUniqueConstraint($type, $properties)
        {
            $tableNoQ = $this->esc($type, true);
            $name =
                "UQ_" . $this->esc($type, true) . implode("__", $properties);
            $t = $this->getTable($type);
        ...
        {
            if (
                $sqlSelectSnippet === AQueryWriter::C_SELECT_SNIPPET_FOR_UPDATE
            ) {
                $sqlSelectSnippet = "";
            }
            $this->sqlSelectSnippet = $sqlSelectSnippet;
...
namespace RedBeanPHP\QueryWriter {
    ...
    class PostgreSQL extends AQueryWriter implements QueryWriter
    {
        ...
        public function createTable($type)
        {
            ...
            $this->adapter->exec(
                sprintf($this->getDDLTemplate("createTable", $type), $table),
            );
        }
...
namespace RedBeanPHP\QueryWriter {
    ...
    class CUBRID extends AQueryWriter implements QueryWriter
    {
        ...
        protected function buildFK(
            ...
            if (
                !is_null(
                    $this->getForeignKeyForTypeProperty($tableNoQ, $columnNoQ),
                )
            ) {
                return false;
            }
        ...
        public function __construct(Adapter $adapter)
        {
            ...
            $this->sqltype_typeno["STRING(1073741823)"] =
                self::C_DATATYPE_STRING;
        ...
        public function addColumn($type, $column, $field)
        {
            ...
            $type = array_key_exists($type, $this->typeno_sqltype)
                ? $this->typeno_sqltype[$type]
                : "";
        ...
        public function addUniqueConstraint($type, $properties)
        {
            ...
            $name = "UQ_" . sha1(implode(",", $columns));
            $sql =
                "ALTER TABLE $table ADD CONSTRAINT UNIQUE $name (" .
                implode(",", $columns) .
                ")";
            try {
                $this->adapter->exec($sql);
...
namespace RedBeanPHP {
    ...
    abstract class Repository
    {
         ...
         * @return void
         */
        protected function processEmbeddedBean(
            &$embeddedBeans,
            $bean,
            $property,
            OODBBean $value
        ...
        {
            try {
                $cursor = $this->writer->queryRecordWithCursor(
                    $type,
                    $sql,
                    $bindings,
                );
        ...
        public function count($type, $addSQL = "", $bindings = [])
        {
            ...
            try {
                $count = (int) $this->writer->queryRecordCount(
                    $type,
                    [],
                    $addSQL,
                    $bindings,
            ...
            if (!$this->oodb->isChilled($bean->getMeta("type"))) {
                ...
                if ($doFKStuff) {
                    if (strrpos($columnNoQ, "_id") === strlen($columnNoQ) - 3) {
                        ...
                        $typeof = $bean->getMeta(
                            "sys.typeof.{$destinationColumnNoQ}",
                            $destinationColumnNoQ,
                        );
                        $isLink = $bean->getMeta(
                            "sys.buildcommand.unique",
                            false,
                        );
                        //Make FK CASCADING if part of exclusive list (dependson=typeof) or if link bean
                        $isDep =
                            $bean->moveMeta("sys.buildcommand.fkdependson") ===
                                $typeof || is_array($isLink);
                        $result = $this->writer->addFK(
                            $table,
                            $typeof,
                            $columnNoQ,
                            "id",
...
namespace RedBeanPHP {
    ...
    class Finder
    {
        ...
        {
            $types = [$parentName, $childName];
            sort($types);
            $link = implode("_", $types);
            return [
                "a" => $parentName,
                "b" => $childName,
                "matcher" => function ($parent, $child, $beans) use (
                    $parentName,
                    $childName,
                    $link
                ) {
                    ...
                    foreach ($beans[$link] as $linkBean) {
                        if (
                            $linkBean["{$parentName}ID"] == $parent->id &&
                            $linkBean["{$childName}ID"] == $child->id
                        ) {
                            return true;
                        }
                    }
        ...
        public static function onMap($parentName, $childNameOrBeans)
        {
            return [
                "a" => $parentName,
                "b" => $childNameOrBeans,
                "matcher" => [$parentName, "{$parentName}_id"],
                "do" => "match",
            ];
        }
...
namespace RedBeanPHP {
    ...
    class TagManager
    {
         ...
         * @return integer
         */
        public function countTaggedAll(
            $beanType,
            $tagList,
            $sql = "",
            $bindings = []
...
namespace RedBeanPHP {
    ...
    class Facade
    {
         ...
         * @return ToolBox
         */
        public static function createToolbox(
            $dsn = null,
            $username = null,
            $password = null,
            $frozen = false,
        ...
        public static function loadForUpdate($type, $id)
        {
            return self::load(
                $type,
                $id,
                AQueryWriter::C_SELECT_SNIPPET_FOR_UPDATE,
            );
        }
         ...
         * @return array
         */
        public static function findForUpdate($type, $sql = null, $bindings = [])
        {
            return self::find(
                $type,
                $sql,
         ...
         * @return OODBBean|NULL
         */
        public static function findOneForUpdate(
            $type,
            $sql = null,
            $bindings = []
        ) {
        ...
        {
            return DispenseHelper::dispenseAll(
                self::$redbean,
                $order,
                $onlyArrays,
            );
         ...
         * @return OODBBean
         */
        public static function findOneOrDispense(
            $type,
            $sql = null,
            $bindings = []
        ) {
         ...
         * @return array
         */
        public static function findMulti(
            $types,
            $sql,
            $bindings = [],
            $remappings = []
        ) {
            return self::$finder->findMulti(
                $types,
                $sql,
                $bindings,
                $remappings,
         ...
         */
        public static function getCursor($sql, $bindings = [])
        {
         ...
         * @return array
         */
        public static function findFromSQL(
            $type,
            $sql,
            $bindings = [],
            $metamask = "extra_",
            ...
            $beans = [];
            if (count($rows)) {
                $beans = self::$redbean->convertToBeans(
                    $type,
                    $rows,
                    $metamask,
                );
         ...
         * @return array
         */
        public static function tagged(
            $beanType,
            $tagList,
            $sql = "",
            $bindings = []
        ) {
            return self::$tagManager->tagged(
                $beanType,
                $tagList,
                $sql,
                $bindings,
         ...
         * @return array
         */
        public static function taggedAll(
            $beanType,
            $tagList,
            $sql = "",
            $bindings = []
        ) {
            return self::$tagManager->taggedAll(
                $beanType,
                $tagList,
                $sql,
                $bindings,
         ...
         * @return integer
         */
        public static function countTaggedAll(
            $beanType,
            $tagList,
            $sql = "",
            $bindings = []
        ) {
            return self::$tagManager->countTaggedAll(
                $beanType,
                $tagList,
                $sql,
                $bindings,
         ...
         * @return integer
         */
        public static function countTagged(
            $beanType,
            $tagList,
            $sql = "",
            $bindings = []
        ) {
            return self::$tagManager->countTagged(
                $beanType,
                $tagList,
                $sql,
                $bindings,
        ...
        public static function loadJoined(
            $beans,
            $type,
            $sqlTemplate = "SELECT %s.* FROM %s WHERE id IN (%s)"
        ) {
            if (!count($beans)) {
                return [];
            }
            ...
            $result = self::findMulti(
                ...
                self::genSlots(
                    $beans,
                    sprintf($sqlTemplate, $type, $type, "%s"),
                ),
                $ids,
         ...
         * @return int
         */
        public static function hunt($type, $sqlSnippet = null, $bindings = [])
        {
        ...
        public static function findOrCreate(
            ...
            return self::$finder->findOrCreate(
                $type,
                $like,
                $sql = "",
                $hasBeenCreated,
         ...
         * @return mixed
         */
        public static function matchUp(
            $type,
            $sql,
            $bindings = [],
            $onFoundDo = null,
        ...
        public static function look(
            $sql,
            $bindings = [],
            $keys = ["selected", "id", "name"],
            $template = '<option %s value="%s">%s</option>',
            $filter = "trim",
            $glue = ""
        ) {
            return self::getLook()->look(
                $sql,
                $bindings,
                $keys,
                $template,
        ...
        public static function countChildren(
            ...
            $sql = null,
            $bindings = [],
            $select = QueryWriter::C_CTE_SELECT_COUNT
        ) {
    ...
    class DuplicationManager
    {
        ...
        public function camelfy($array, $dolphinMode = false)
        {
            ...
            foreach ($array as $key => $element) {
                ...
                $newArray[$newKey] = is_array($element)
                    ? $this->camelfy($element, $dolphinMode)
                    : $element;
            }
            return $newArray;
        }
...
namespace RedBeanPHP\Util {
    ...
    class DispenseHelper
    {
         ...
         * @return array
         */
        public static function dispenseAll(
            OODB $oodb,
            $order,
            $onlyArrays = false
        ) {
            ...
            if ($bean && $onFoundDo) {
                foreach ($onFoundDo as $property => $value) {
                    if (function_exists("is_callable") && is_callable($value)) {
                        $bean[$property] = call_user_func_array($value, [
                            $bean,
                        ]);
                    } else {
                        $bean[$property] = $value;
    ...
    class Look
    {
        ...
        public function look(
            ...
            foreach ($rows as $row) {
                ...
                foreach ($keys as $key) {
                    if (!empty($filter)) {
                        $values[] = call_user_func_array($filter, [$row[$key]]);
                    } else {
                        $values[] = $row[$key];
                    }
                }
        ...
        public function diff(
            ...
            foreach ($beansI as $id => $bean) {
                ...
                foreach ($bean as $property => $value) {
                    ...
                    $key = vsprintf($format, [$type, $bean->id, $property]);
...
namespace RedBeanPHP\Util {
    ...
    class Tree
    {
         ...
         * @return array
         */
        public function children(OODBBean $bean, $sql = null, $bindings = [])
        {
            $type = $bean->getMeta("type");
            $id = $bean->id;

            $rows = $this->writer->queryRecursiveCommonTableExpression(
                $type,
                $id,
                false,
                $sql,
         ...
         * @return array
         */
        public function parents(OODBBean $bean, $sql = null, $bindings = [])
        {
            $type = $bean->getMeta("type");
            $id = $bean->id;
